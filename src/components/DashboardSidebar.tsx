...
const DashboardSidebar = ({ isOwner = false }) => {
  ...
  return (
    <aside className="w-64 bg-gradient-to-br from-[#1a1528] to-[#0f0a1e] h-screen fixed left-0 top-16">
      <nav className="p-4">
        <ul className="space-y-2">
          {menuItems.map((item) => (
            <li key={item.path}>
              <Link
                to={item.path}
                className="flex items-center gap-3 px-4 py-2.5 rounded-lg text-gray-300 hover:text-white hover:bg-white/10 transition-all duration-300 group"
              >
